...
export default function MapaProfissional() {
  ...
  useEffect(() => {
    ...
    const carregarListaAreasDestaque = async () => {
      try {
        const docRef = doc(db, "listas", "areasDestaque");
        const snap = await getDoc(docRef);
        if (snap.exists()) {
          setListaAreasDestaque(snap.data().valores || []);
        }
      } catch (error) {
        console.error("Erro ao carregar áreas de destaque:", error);
      }
    };
    carregarListaProfissoes();
    carregarListaAreasDestaque();
  }, []);
  ...
  const adicionarArea = (area: string) => {
    if (areasDestaque.length >= 10) return;
    if (area.length > 20) return;
      ...
      <div>
        ...
        <div>
          {areasDestaque.map((area, idx) => (
            <span key={idx} style={{ marginRight: "10px" }}>{area}</span>
          ))}
        </div>
      </div>
    </div>
